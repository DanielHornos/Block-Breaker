...
public class BlockBreakerPanel extends JPanel implements KeyListener{
	...
	public BlockBreakerPanel() {
		paddle = new Block(175, 480, 150, 25, "resources/paddle.png");
		for(int i = 0; i<8; i++) {
			blocks.add(new Block((i*60+2), 0, 60, 25, "resources/blue.png"));
		}
		for(int i = 0; i<8; i++) {
			blocks.add(new Block((i*60+2), 25, 60, 25, "resources/red.png"));
		}
		for(int i = 0; i<8; i++) {
			blocks.add(new Block((i*60+2), 50, 60, 25, "resources/green.png"));
		}
		for(int i = 0; i<8; i++) {
			blocks.add(new Block((i*60+2), 75, 60, 25, "resources/yellow.png"));
		}
		...
		ball.add(new Block(237, 437, 25, 25, "resources/ball.png"));
		addKeyListener(this);
		setFocusable(true);
	}
	...
	public void update() {
		for(Block p : powerup) {
			...
			if(p.intersects(paddle) && !p.destroyed) {
				p.destroyed=true;
				ball.add(new Block(paddle.dx+75, 437, 25, 25, "resources/ball.png"));
			}
		}
		...
		for(Block ba : ball) {
			...
			for(Block b : blocks) {
				if((b.left.intersects(ba) || b.right.intersects(ba)) && !b.destroyed) {
					...
					ba.dx*=-1;
					if(b.powerup) {
						powerup.add(new Block(b.x, b.y, 25, 19, "resources/extra.png"));
					}
				}
